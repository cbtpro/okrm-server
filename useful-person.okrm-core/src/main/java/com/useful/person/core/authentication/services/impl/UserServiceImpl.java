...
public class UserServiceImpl implements IUserService {
    ...
    public void updateUserPassword(String uuid, String oldPassword, String newPassword) {
        ...
        if (passwordEncoder.matches(oldPassword, userInfo.getPassword())) {
            ...
            UserInfoLog userInfoLog = UserInfoLog.builder().actionType(UserAction.UPDATE_PASSWORD).user(userInfo)
                    .build();
            userInfoLogRepository.save(userInfoLog);
        } else {
    ...
    public void updateMobileByUuid(String uuid, String mobile) {
        ...
        if (userInfo == null) {
            ...
            UserInfoLog userInfoLog = UserInfoLog.builder().user(currentUser).actionType(UserAction.UPDATE_MOBILE)
                    .oldValue(null).actionValue(mobile).build();
            userInfoLogRepository.save(userInfoLog);
        } else {
            throw new MobileExistException(mobile);
        }
    ...
    public void updateEmailByUuid(String uuid, String email) {
        ...
        if (userInfo == null) {
            ...
            UserInfo currentUser = UserInfo.builder().uuid(uuid).build();
            UserInfoLog userInfoLog = UserInfoLog.builder().user(currentUser).actionType(UserAction.UPDATE_EMAIL)
                    .oldValue(null).actionValue(email).build();
            userInfoLogRepository.save(userInfoLog);
        } else {
    ...
    public void unbindOldMobile(String uuid, String mobile) {
        ...
        if (userInfo != null && uuid.equalsIgnoreCase(userInfo.getUuid()) && mobile.equals(userInfo.getMobile())) {
            ...
            UserInfoLog userInfoLog = UserInfoLog.builder().user(userInfo).actionType(UserAction.UPDATE_MOBILE)
                    .actionValue(null).oldValue(mobile).build();
            userInfoLogRepository.save(userInfoLog);
        } else {
    ...
    public void unbindOldEmail(String uuid, String email) {
        ...
        if (userInfo != null && uuid.equalsIgnoreCase(userInfo.getUuid()) && email.equals(userInfo.getEmail())) {
            ...
            UserInfoLog userInfoLog = UserInfoLog.builder().user(userInfo).actionType(UserAction.UPDATE_EMAIL)
                    .actionValue(null).oldValue(email).build();
            userInfoLogRepository.save(userInfoLog);
        } else {
    ...
    public void updateRealname(String uuid, String identityCardName, String identityCardNo) {
        ...
        UserInfoLog userInfoLog = UserInfoLog.builder().user(userInfo).actionType(UserAction.UPDATE_IDENTITY_CARD)
                .oldValue(null).actionValue(userInfo.getIdentityCardName() + "," + userInfo.getIdentityCardNo())
                .build();
        userInfoLogRepository.save(userInfoLog);
    }
    ...
    @Override
    public List<Address> getUserNearbyUserAddress(String uuid, Double longitude0, Double latitude0, Double longitude1,
            Double latitude1, Double longitude, Double latitude) {
        ...
        /**
         * 判断是否需要拆分区域 经度170 -> 180 -> (-170)需要拆分成 170 -> 180/ (-180) -> (-170)
        ...
        if (minLongitude > maxLongitude) {
            ...
            return userInfoRepository.queryUserNearByUserAddressRange(range0MinLongitude, range0MaxLongitude,
                    range1MinLongitude, range1MaxLongitude, minLatitude, maxLatitude);
        }
    ...
    @Override
    @Transactional
    public UserInfo updateUserInfo(String uuid, String avatar, String username, String nickname, String province,
            String city, String county, Timestamp birthday) {
        UserInfo userInfo = userInfoRepository.findById(uuid).orElseThrow(() -> new GeneralException("", "更新信息失败！"));
        int count = userInfoRepository.updateUserInfo(uuid, avatar, username, nickname, province, city, county,
                birthday);
        if (count > 0) {
            UserInfoLog.builder().actionType(UserAction.UPDATE_AVATAR).actionValue(avatar)
                    .oldValue(userInfo.getAvatar()).build();
            UserInfoLog.builder().actionType(UserAction.UPDATE_USERNAME).actionValue(username)
                    .oldValue(userInfo.getUsername()).build();
            UserInfoLog.builder().actionType(UserAction.UPDATE_NICKNAME).actionValue(nickname)
                    .oldValue(userInfo.getNickname()).build();
            Timestamp oldBirthday = userInfo.getBirthday();
            String oldBirthDayStr = oldBirthday != null ? oldBirthday.toString() : null;
            UserInfoLog.builder().actionType(UserAction.UPDATE_BIRTHDAY).actionValue(birthday.toString())
                    .oldValue(oldBirthDayStr).build();
